...
## Mirror Draw minigame.
## An arrow sequence is shown, then input it in REVERSE order.
## Sequence length grows as you progress.
## Score = number of sequences reversed. Race to 8.
...
var _reversed_sequence: Array[String] = []
...
func _ready() -> void:
	...
	feedback_label.text = ""
	input_label.text = ""
	score_label.text = "Reversed: 0 / " + str(COMPLETION_TARGET)
...
func _on_game_end() -> void:
	_phase = "waiting"
	feedback_label.text = "Time's up! You reversed " + str(_sequences_completed) + " sequences!"
	submit_score(_sequences_completed)


func _generate_and_show_sequence() -> void:
	var length: int = STARTING_LENGTH + (_sequences_completed / 3)
	_current_sequence.clear()
	_reversed_sequence.clear()
	for i: int in range(length):
		_current_sequence.append(DIRECTIONS[randi_range(0, DIRECTIONS.size() - 1)])

	# Build reversed sequence
	for i: int in range(_current_sequence.size() - 1, -1, -1):
		_reversed_sequence.append(_current_sequence[i])
...
func _show_next_arrow() -> void:
	if _show_index >= _current_sequence.size():
		...
		_phase = "input"
		_player_input_index = 0
		# Show the full sequence as reference, then ask for reverse
		var full_seq: String = ""
		for i: int in range(_current_sequence.size()):
			if full_seq.length() > 0:
				full_seq += " "
			full_seq += ARROW_SYMBOLS[_current_sequence[i]]
		sequence_label.text = full_seq
		status_label.text = "Now input it BACKWARDS!"
		input_label.text = ""
		return
...
func _unhandled_input(event: InputEvent) -> void:
	...
	var expected: String = _reversed_sequence[_player_input_index]
	if pressed_direction == expected:
		...
		for i: int in range(_player_input_index):
			if progress.length() > 0:
				progress += " "
			progress += ARROW_SYMBOLS[_reversed_sequence[i]]
		input_label.text = progress
		feedback_label.text = "Correct!"

		if _player_input_index >= _reversed_sequence.size():
			# Sequence reversed successfully
			_sequences_completed += 1
			score_label.text = "Reversed: " + str(_sequences_completed) + " / " + str(COMPLETION_TARGET)
			feedback_label.text = "Sequence reversed!"
