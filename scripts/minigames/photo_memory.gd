...
## Photo Memory minigame.
## Grid of face-down cards with icon pairs. Flip two at a time to find matches.
## Race to clear the board (all pairs found).
## Score = number of pairs found within 10 seconds.
...
const NUM_PAIRS: int = TOTAL_CARDS / 2
const COMPLETION_TARGET: int = NUM_PAIRS  # Clear the whole board

## Symbols used as card "icons"
...
## Colors for each symbol (so matched pairs are visually distinct)
...
var _score: int = 0
var _card_values: Array[int] = []  # Symbol index for each card position
var _card_buttons: Array[Button] = []
var _revealed: Array[bool] = []  # Which cards are permanently revealed (matched)
var _first_pick: int = -1  # Index of first flipped card (-1 = none)
var _second_pick: int = -1  # Index of second flipped card (-1 = none)
var _checking: bool = false  # Whether we're in the delay after flipping two cards
...
func _generate_board() -> void:
	...
	# Pick NUM_PAIRS random symbols and create pairs
	var available_symbols: Array[int] = []
	for i: int in range(SYMBOLS.size()):
		available_symbols.append(i)

	# Shuffle and pick first NUM_PAIRS
	for i: int in range(available_symbols.size() - 1, 0, -1):
		var j: int = randi() % (i + 1)
		var tmp: int = available_symbols[i]
		available_symbols[i] = available_symbols[j]
		available_symbols[j] = tmp

	# Create pairs
	for i: int in range(NUM_PAIRS):
		var sym_idx: int = available_symbols[i]
		_card_values.append(sym_idx)
		_card_values.append(sym_idx)
...
func _show_card(index: int) -> void:
	...
	var sym_idx: int = _card_values[index]
	var color_idx: int = sym_idx % SYMBOL_COLORS.size()
	var bg_color: Color = SYMBOL_COLORS[color_idx]
...
func _set_card_face_down(btn: Button) -> void:
	btn.text = "?"
	var stylebox: StyleBoxFlat = StyleBoxFlat.new()
	stylebox.bg_color = Color(0.3, 0.35, 0.45)
	stylebox.corner_radius_top_left = 4
	stylebox.corner_radius_top_right = 4
	stylebox.corner_radius_bottom_left = 4
	stylebox.corner_radius_bottom_right = 4
...
func _set_card_matched(btn: Button) -> void:
	...
	btn.disabled = true
	var stylebox: StyleBoxFlat = StyleBoxFlat.new()
	stylebox.bg_color = Color(0.15, 0.2, 0.15, 0.5)
	stylebox.corner_radius_top_left = 4
	stylebox.corner_radius_top_right = 4
	stylebox.corner_radius_bottom_left = 4
	stylebox.corner_radius_bottom_right = 4
...
func _update_score_display() -> void:
	score_label.text = "Pairs: " + str(_score)
